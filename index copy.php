...
$achieved_target = $_SESSION['achieved_target'];
...
<section class="main_content dashboard_part large_header_bg">
    ...
    if ($result->num_rows > 0) {
        ...
        // print_r($rows);
        // exit;
        if ($GLOBALS['achieved_target'] < $GLOBALS['target']) {
            // echo "true";
            function rule_check($string)
            {
                preg_match_all('/([><=]=|[><=*+-])\s*(-?\d+(\.\d+)?)/', $string, $symbols, PREG_SET_ORDER);
                if (!empty($symbols)) {
                    foreach ($symbols as $symbol) {
                        $operator = $symbol[1]; // Arithmetic operator
                        $number = $symbol[2]; // Number
            ...
            foreach ($rows as $row) {
                // $rule = $row["rule"];
                $ruleJsonString = $row["rule"];
                // echo $ruleJsonString;
                // exit;
                // Decode the JSON string into a PHP object
                $ruleData = json_decode($ruleJsonString);
                // var_dump($ruleData);
                // exit;
                // Access the properties of the JSON data
                $education_rule = $ruleData->education;
                // echo $education;
                // exit;
                $age_rule = $ruleData->age;
                $rule = $ruleData->rule;
                $emp_age = $GLOBALS['age'];
                $emp_education = $GLOBALS['education'];
                $age_result = rule_check($age_rule);
                // echo $age_result['number'];
                $age_data = $age_result['number'];
                $age_operstor = $age_result['operator'];
                //check functionality
                $expression = "$age_data" . "$age_operstor" . "$emp_age";
                $result_emp_age = eval("return 1;");
                // if($result === true)
                
                // echo "result".$result."<br>";
                // var_dump($result);
                if($result_emp_age == 1){
                    $final_rule_age = $age_data;
                    // echo $final_rule_age."<br>";
                    $education_result = rule_check($education_rule);
                    // var_dump(($education_result));
                    $education_operator = $education_result['operator'];
                    $education_data = $education_result['number'];
                    $expression = "$education_data" . "$education_operator" . "$emp_education";
                    $result_emp_education = eval("return 1;");
                    if($result_emp_age == 1 && $result_emp_education == 1){
                        echo $rule;
                        break;
                    }
                ...
                exit;
                echo "age_data".$age_data."<br>";
                echo "age_operstor".$age_operstor."<br>";
                echo "emp_age".$emp_age."<br>";
                // if ($result === false) {
                ...
                // echo $age_result['number'];
                // echo "<br>";
                // print_r($age_result['number']);
                // echo "<br>";
                // if (is_array($age_result) || is_object($age_result)){
                //     foreach($age_result as $data){
                //         echo "hi<br>";
                //         print_r($age_result['number']);
                //         echo "<br>";
                //         exit;
                ...
                //         echo "hi";
                //         echo "<br>";
                //         $age_data_num = $data['number'];
                //         echo $age_data_num;
                //         echo "<br>";
                //         $age_operstor = $data['operator'];
                //         $abbas = $data['rule'];
                //         echo $abbas;
                //         print_r($data);
                //     }
                ...
                //  $age_data = $age_result['number'];
                // $age_operstor = $age_result['operator'];
                // $expression = "$age_data" . "$age_operstor" . "30";
